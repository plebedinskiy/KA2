...
Процедура RkНаСервере(Адрес)
    ...
    Объект.ПутьКФайлуНаСервере = ПолучитьИмяВременногоФайла();
...
Процедура ПрочитатьФайлМеталлоконструкцииНаСервере()
	...
	Пока Строка <> Неопределено Цикл //строки читаются до символа перевода строки
		...
		Если СтрНачинаетсяС(ОбработаннаяСтрока, "на сварные швы") 		Тогда Продолжить; КонецЕсли;
		...
		Если МассивИзОбработаннойСтроки.Количество() < 2 Тогда Продолжить; КонецЕсли;
		...
		СтрокаДляМеталлоконструкции.НазваниеМатериала			 = МассивИзОбработаннойСтроки[1];
