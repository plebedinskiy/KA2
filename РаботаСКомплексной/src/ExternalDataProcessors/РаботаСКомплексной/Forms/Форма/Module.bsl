...
Процедура ПрочитатьФайл(Команда)
	...
	КонецЕсли;
	//ТабличныйДокумент = 
	ЗагрузитьМатериалыВСтроительнуюРаботуНаСервере();
		...
		Для Каждого СтрокаИзМеталлоконструкций Из ТЗ_Металлоконструкций Цикл
			...
			Если НЕ СтрокаИзМеталлоконструкций.НазваниеМеталлоконструкции = ПрошлоеНазваниеМеталлоконструкции Тогда
				...
				Если НЕ ПрошлоеНазваниеМеталлоконструкции = "" Тогда
					...
					ЕдиницаИзмерения = СоздатьИВернутьИндивидуальнуюЕдиницуИзмерения(МеталлоконструкцияСсылка, ОбщийВесГП, ВесПрошлойМеталлоконструкции);
					ВключитьВВозвратныеОтходыСпецификацииНоменклатуруМПЗ(Спецификация, ОбщийВесМеталлолома, ЭтапСпецификации);
					ЗаписатьВесВРесурснуюСпецификацию(Спецификация, МеталлоконструкцияСсылка, ЭтапСпецификации, ОбщийВесГП, ВесПрошлойМеталлоконструкции, ЕдиницаИзмерения);
					  
					Спецификация.Статус = Перечисления.СтатусыСпецификаций.Действует;
					Спецификация.Записать();
					ЗаписатьВесВНоменклатуруГП(МеталлоконструкцияСсылка, ОбщийВесГП, ВесПрошлойМеталлоконструкции);
					ДополнитьЗаказНоменклатуройГП(ЗаказДокумент, МеталлоконструкцияСсылка, КоличествоПрошлойМеталлоконструкции, ВесПрошлойМеталлоконструкции, ЕдиницаИзмерения);
				...
				Спецификация 		= СоздатьИВернутьСпецификацию(СтрокаИзМеталлоконструкций);
...
&НаСервере
Функция  СоздатьИВернутьИндивидуальнуюЕдиницуИзмерения(МеталлоконструкцияСсылка, ОбщийВесГП, ВесПрошлойМеталлоконструкции)
...
&НаСервере
Функция СоздатьИВернутьСпецификацию(СтрокаИзМеталлоконструкций)
...
&НаСервере
Процедура ЗаписатьВесВРесурснуюСпецификацию(Спецификация, МеталлоконструкцияСсылка, Этап, ОбщийВесГП, ВесПрошлойМеталлоконструкции, ЕдиницаИзмерения)
		
	СтрокаВСпецификацииВыходныеИзделия 									= Спецификация.ВыходныеИзделия.Добавить();
	СтрокаВСпецификацииВыходныеИзделия.Номенклатура 					= МеталлоконструкцияСсылка;
...
&НаСервере
Процедура ВключитьВВозвратныеОтходыСпецификацииНоменклатуруМПЗ(Спецификация, ОбщийВесМеталлолома, Этап)
...
&НаСервере
Процедура ЗаписатьВесВНоменклатуруГП(МеталлоконструкцияСсылка, ОбщийВесГП, ВесПрошлойМеталлоконструкции)
...
&НаСервере
Процедура ДополнитьЗаказНоменклатуройГП(ЗаказДокумент, МеталлоконструкцияСсылка, КоличествоПрошлойМеталлоконструкции, ВесПрошлойМеталлоконструкции, ЕдиницаИзмерения)
	
	СтрокаВЗаказе = ЗаказДокумент.Товары.Добавить();
	
	СтрокаВЗаказе.Номенклатура 			= МеталлоконструкцияСсылка;
	СтрокаВЗаказе.Упаковка 				= ЕдиницаИзмеренияТонна;
	СтрокаВЗаказе.Количество 			= ВесПрошлойМеталлоконструкции / 1000 * КоличествоПрошлойМеталлоконструкции;
	СтрокаВЗаказе.КоличествоУпаковок 	= ВесПрошлойМеталлоконструкции / 1000 * КоличествоПрошлойМеталлоконструкции;
...
Процедура ЗаполнитьНаСервереДляКА2()
	...
	ЗаполнитьНоменклатуруВСоответствии();
	ОбработатьДанныеСоответствияПоНоменклатуре();
	ЭтоУспех = ВернутьРезультатПроверкиНесопоставленныхМеханизмов(ЭтоУспех);
	Если НЕ ЭтоУспех Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ВернутьРезультатПроверкиНесопоставленныхМеханизмов(ЭтоУспех)
	
	ОтборПоПустымСтрокам = Новый Структура("Номенклатура", Справочники.Номенклатура.ПустаяСсылка());
	МассивСтрок = Объект.СоответствиеПоНоменклатуре.НайтиСтроки(ОтборПоПустымСтрокам);
	
	КоличествоСтрок = МассивСтрок.Количество();
	Индекс = 0;
	Пока Индекс <= КоличествоСтрок Цикл
		Если СтрНайти(МассивСтрок[Индекс].НоменклатураСтрокой, "00000") > 0 Тогда
			Сообщить("В справочнике номенклатуры отсутствует механизм " + МассивСтрок[Индекс].НоменклатураСтрокой);
			ЭтоУспех = Ложь;
		КонецЕсли;
	КонецЦикла;
	
	ЭтоУспех = ?(ЭтоУспех = Ложь, Ложь, ЭтоУспех);
...
Процедура ЗаполнитьНоменклатуруВСоответствии()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НРег(Выборка.ТипНоменклатурыСтрокой) = НРег("Работа") Тогда
			Продолжить;		
		КонецЕсли;
	...
	Для Сч = НомерСтроки по КолСтр Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(Колонка3) И ЗначениеЗаполнено(Колонка12) И ЗначениеЗаполнено(Колонка13)
			...
			Если ВРег(Колонка2) = "МПЗ" Тогда
				СтрокаДляДобавления.НоменклатураСтрокой 		= "";
			ИначеЕсли ВРег(Колонка2) = "РАБОТА" Тогда
				СтрокаДляДобавления.НоменклатураСтрокой			= Объект.КодПроекта + " ";
			ИначеЕсли ВРег(Колонка2) = "МЕХАНИЗМ" Тогда
				СтрокаДляДобавления.НоменклатураСтрокой 		= ?(СтрНайти(Колонка3,"00000") > 0, "", "00000 ");
			КонецЕсли;
			СтрокаДляДобавления.НоменклатураСтрокой 			= СтрокаДляДобавления.НоменклатураСтрокой + Колонка3;
